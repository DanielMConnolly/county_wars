...
const PlacementInfoCard = () => {
  ...
  const [projectedIncome, setProjectedIncome] = useState<number | null>(null);
  ...
  useEffect(() => {
    const fetchLocationInformation = async () => {
      ...
      // Fetch potential income if in franchise mode
      if (placementMode === 'franchise') {
        try {
          const potentialIncomeData = await fetchPotentialIncome(clickedLocation.lat, clickedLocation.lng);
          if (potentialIncomeData.success) {
            setProjectedIncome(potentialIncomeData.projectedIncome || 0);
          }
        } catch (error) {
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <>
            ...
            {placementMode === 'franchise' && projectedIncome !== null && (
              <InfoRow
                label="Projected Income:"
                value={`$${projectedIncome}/turn`}
