...
import { calculateDifficultyScoreBasedOnPopulation, getCost, useIsCountyOwned, getDifficultyLevel, getDifficultyColor } from './utils/gameUtils';
...
const InfoCard = () => {
  ...
  const getButtonState = () => {
    ...
    if (ownedCounties.has(selectedCounty.countyFP + selectedCounty.stateFP)) return { text: 'Already Owned', disabled: true };
  ...
  return (
    <div className="fixed bottom-6 right-6 w-80 bg-gradient-to-br from-slate-800 to-slate-900 backdrop-blur-sm rounded-xl p-6 z-[1000] border border-slate-600 shadow-2xl">
